...
namespace win32 {
  ...
  ProcessSessionId::ProcessSessionId(DWORD processId) {
    ...
    vlog.debug("[ProcessSessionId] processId(%lu), session_id(%lu)", processId, id);
  }
  ...
  bool inConsoleSession() {
    ConsoleSessionId console;//active session
    vlog.debug("[inConsoleSession] console(%lu), mySessionId(%lu)", console.id, mySessionId.id);
  ...
  void setConsoleSession(DWORD sessionId) {
    ...
    vlog.info("Console session is %lu, sessionId(%lu)", console.id, sessionId);
