...
static rfb::LogWriter vlog("network_Socket");
...
void Socket::setFd(int fd)
{
  ...
  isShutdown_ = false;
  vlog.debug("socket instream bufsize(%d), start(%p), offset(%d), peakUsage(%d), ptr(%p), end(%p)", 
              (int)instream->GetBufSize(), instream->GetStartPointer(),(int)instream->GetOffset(),(int)instream->GetPeakUsage(),
              instream->GetPtrPointer(), instream->GetEndPointer());
}
