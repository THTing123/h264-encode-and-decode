...
static const int EncodeRectMinWidth = 600;
...
{
  vlog.debug("writeLosslessRefresh pb->width(%d), pb->height(%d)", pb->width(), pb->height());
                             ...
                             const RenderedCursor* renderedCursor)
{
    ...
    if (conn->client.supportsEncoding(encodingCopyRect)){
      vlog.debug("doUpdate --debug-- client supportsEncoding encodingCopyRect");
     ...
     */
    if (conn->client.supportsEncoding(pseudoEncodingLastRect)){
      vlog.debug("doUpdate --debug-- client supportsEncoding pseudoEncodingLastRect");
      writeSolidRects(&changed, pb);
    }

    writeRects(changed, pb);
    vlog.debug("between changed and cursorRegion");
...
void EncodeManager::prepareEncoders(bool allowLossy)
{
  ...
  case encodingTight:
    ...
    break;
  case encodingZRLE:
    vlog.debug("prepareEncoders --- encodingZRLE");
  ...
  vlog.debug("prepareEncoders --- solid(%d), bitmap(%d), bitmapRLE(%d),indexed(%d),indexedRLE(%d),fullColour(%d)",solid, bitmap, bitmapRLE, indexed,indexedRLE,fullColour);
  ...
  vlog.debug("prepareEncoders --- fullColour(%d), preferred(%d)",fullColour, preferred);
...
Encoder *EncodeManager::startRect(const Rect& rect, int type)
{
  ...
  vlog.debug("EncodeManager::startRect type(%d), klass(%d) relavent to encoder type", type, klass);
...
void EncodeManager::writeSolidRects(Region *changed, const PixelBuffer* pb)
{
  ...
  vlog.debug("writeSolidRects pb->width(%d), pb->height(%d)", pb->width(), pb->height());
  changed->get_rects(&rects);
  for (rect = rects.begin(); rect != rects.end(); ++rect)
    ...
    for (dx = rect.tl.x; dx < rect.br.x; dx += SolidSearchBlock) {
      ...
      if (checkSolidTile(sr, colourValue, pb)) {
        ...
        if (encoder->flags & EncoderUseNativePF) {
          ...
          vlog.debug("findSolidRect erp.width(%d), erp.height(%d)", erp.width(), erp.height());
          encoder->writeSolidRect(erp.width(), erp.height(),
...
void EncodeManager::writeRects(const Region& changed, const PixelBuffer* pb)
{
  ...
  for (rect = rects.begin(); rect != rects.end(); ++rect) {
    ...
    vlog.debug("[writeRects] rect_size(%d), w(%d), h(%d), rect.tl.(x,y)=(%d,%d),rect.br.(x,y)=(%d,%d)", (int)rects.size(), w, h, rect->tl.x, rect->tl.y, rect->br.x, rect->br.y);
    // No split necessary?
    if (((w*h) < SubRectMaxArea) && (w < SubRectMaxWidth)) {
    ...
    for (sr.tl.y = rect->tl.y; sr.tl.y < rect->br.y; sr.tl.y += sh) {
      ...
      for (sr.tl.x = rect->tl.x; sr.tl.x < rect->br.x; sr.tl.x += sw) {
        ...
        if (sr.br.x > rect->br.x)
          sr.br.x = rect->br.x;
        cnt++;
        vlog.debug("writeRects (tl.x,tl.y)=(%d,%d), (br.x,br.y)=(%d,%d), cnt(%d)", sr.tl.x, sr.tl.y, sr.br.x, sr.br.y, cnt);
        writeSubRect(sr, pb);
      }
    }
  }
...
void EncodeManager::writeSubRect(const Rect& rect, const PixelBuffer *pb)
{
  ...
  vlog.debug("[writeSubRect] maxColours rect.area()/divisor %d", maxColours);
  ...
  vlog.debug("[writeSubRect] encoderIndexedRLE maxColours(%d), maxPaletteSize(%d)", maxColours, encoder->maxPaletteSize);
  if (maxColours > encoder->maxPaletteSize)
    maxColours = encoder->maxPaletteSize;
  encoder = encoders[activeEncoders[encoderIndexed]];
  vlog.debug("[writeSubRect] encoderIndexed maxColours(%d), maxPaletteSize(%d)", maxColours, encoder->maxPaletteSize);
  if (maxColours > encoder->maxPaletteSize)
    maxColours = encoder->maxPaletteSize;
  ...
  vlog.debug("writeSubRect--- debug preparePixelBuffer ppb->width(%d), ppb->height(%d), encoder->flags(%d)", ppb->width(), ppb->height(), encoder->flags);
  if (encoder->flags & EncoderUseNativePF)
