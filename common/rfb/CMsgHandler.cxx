...
{
  vlog.debug("[setDesktopSize]server setDimensions width(%d), height(%d)", width, height);
...
void CMsgHandler::serverInit(int width, int height,
                             ...
                             const char* name)
{
  vlog.debug("[serverInit]server setDimensions width(%d), height(%d), name(%s)", width, height, name);
  server.setDimensions(width, height);
  server.setPF(pf);
  server.setName(name);
}
