...
void VNCSConnectionST::authSuccess()
{
  ...
  vlog.debug("[VNCSConnectionST::authSuccess] width(%d), height(%d)", server->getPixelBuffer()->width(), server->getPixelBuffer()->height());
  client.setDimensions(server->getPixelBuffer()->width(),
                       server->getPixelBuffer()->height(),
                       server->getScreenLayout());
...
void VNCSConnectionST::writeDataUpdate()
{
  ...
  if (needRenderedCursor()) {
    ...
    vlog.debug("writeDataUpdate renderedCursorRect tl.(x,y) = (%d, %d), br.(x,y) = (%d, %d)", renderedCursorRect.tl.x, renderedCursorRect.tl.y, renderedCursorRect.br.x, renderedCursorRect.br.y);
  ...
  if (ui.is_empty() && !writer()->needFakeUpdate()) {
    vlog.debug("ui.is_empty, writer ->needFakeUpdate()");
...
void VNCSConnectionST::screenLayoutChange(uint16_t reason)
{
  ...
  vlog.debug("[VNCSConnectionST::screenLayoutChange] width(%d), height(%d)", client.width(), client.height());
