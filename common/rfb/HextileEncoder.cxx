...
static LogWriter vlog("HextileEncoder");
...
{
  vlog.debug("HextileEncoder::isSupported");
                               ...
                               const Palette& /*palette*/)
{
  ...
  vlog.debug("writeRect PixelBuffer bpp %d, pb->width(%d), pb->height(%d), run_time(%ld), clk(%d)", pb->getPF().bpp, pb->width(), pb->height(), end_time-star_time, CLK_TCK);
}
                                    ...
                                    const uint8_t* colour)
{
  ...
  vlog.debug("writeSolidRect PixelBuffer, width(%d), height(%d), tiles(%d)", width, height, tiles);
