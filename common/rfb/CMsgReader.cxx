...
bool CMsgReader::readServerInit()
{
  ...
  width = is->readU16();
  height = is->readU16();
  vlog.debug("[CMsgReader::readServerInit] width(%d), height(%d)", width, height);
...
bool CMsgReader::readMsg()
{
  if (state == MSGSTATE_IDLE) {
    ...
    vlog.debug("[CMsgReader::readMsg] state == MSGSTATE_IDLE");
    currentMsgType = is->readU8();
    state = MSGSTATE_MESSAGE;
  }
  vlog.debug("[CMsgReader::readMsg] readMsg state(%d), currentMsgType(%d)", state, currentMsgType);
  ...
  if (currentMsgType != msgTypeFramebufferUpdate) {
    ...
    if (state == MSGSTATE_MESSAGE) {
      ...
      vlog.debug("[CMsgReader::readMsg] state == MSGSTATE_MESSAGE");
    ...
    if (state == MSGSTATE_RECT_HEADER) {
      ...
      vlog.debug("readMsg dataRect.setXYWH x(%d),y(%d),w(%d),h(%d),rectEncoding(%d)", x, y, w, h, rectEncoding);
    ...
    if (ret) {
      state = MSGSTATE_RECT_HEADER;
      nUpdateRectsLeft--;
      vlog.debug("readRect return true, nUpdateRectsLeft(%d)", nUpdateRectsLeft);
      if (nUpdateRectsLeft == 0) {
        state = MSGSTATE_IDLE;
        handler->framebufferUpdateEnd();
      }
...
bool CMsgReader::readFramebufferUpdate()
{
  ...
  handler->framebufferUpdateStart();//CConnection::framebufferUpdateStart
  vlog.debug("readFramebufferUpdate nUpdateRectsLeft(%d)", nUpdateRectsLeft);
