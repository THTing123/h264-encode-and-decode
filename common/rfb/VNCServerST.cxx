                            ...
                            const uint8_t* data)
{
  ...
  slog.debug("[VNCServerST] setCursor width(%d), height(%d)", width, height);
...
void VNCServerST::writeUpdate()
{
  ...
  if (needRenderedCursor()) {
    ...
    slog.debug("[VNCServerST]writeUpdate cursor->width(%d), cursor->height(%d)", cursor->width(), cursor->height());
