  ...
  conn(conn), threadException(NULL)
{
  ...
  vlog.debug("Creating %d decoder thread(s)", (int)cpuCount);
                               ...
                               ModifiablePixelBuffer* pb)
{
  ...
  vlog.debug("[decodeRect] bufferStream start(%p),ptr(%p),end(%p)",bufferStream->GetStartPoint(), bufferStream->GetPtrPoint(),bufferStream->GetEndPoint());
  ...
  vlog.debug("decodeRect start encoding(%d), entry(%p), freebuffer_size(%d)", encoding, entry, (int)freeBuffers.size());
...
void DecodeManager::DecodeThread::worker()
{
  ...
  while (!stopRequested) {
    ...
    try {
      vlog.debug("worker entry_addr(%p), bufferStream start(%p), ptr(%p), length(%d)", 
                  entry, entry->bufferStream->data(), entry->bufferStream->GetPtrPoint(), (int)entry->bufferStream->length());
