...
static rfb::LogWriter vlog("CMsgWriter");
                                     ...
                                     const ScreenSet& layout)
{
  ...
  vlog.debug("[CMsgWriter::writeSetDesktopSize] writeSetDesktopSize width(%d), height(%d), layoutNum(%d)", width, height, layout.num_screens());
  ...
  for (iter = layout.begin();iter != layout.end();++iter) {
    ...
    vlog.debug("writeSetDesktopSize tl(%d, %d), width(%d), height(%d)", iter->dimensions.tl.x, iter->dimensions.tl.y, iter->dimensions.width(), iter->dimensions.height());
  }
