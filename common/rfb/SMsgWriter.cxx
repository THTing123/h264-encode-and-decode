...
{
  vlog.debug("writeServerInit width(%d), height(%d), name(%s)", width, height, name);
  os->writeU16(width);
  os->writeU16(height);
  pf.write(os);
...
void SMsgWriter::writeCopyRect(const Rect& r, int srcX, int srcY)
{
  ...
  os->writeU16(srcY);
  endRect();
  vlog.debug("writeCopyRect r.width(%d), r.height(%d), srcX(%d), srcY(%d)", r.width(), r.height(), srcX, srcY);
}
