...
static rfb::LogWriter mlog("MemOutStream");
...
namespace rdr {
  ...
  protected:
    ...
    virtual void overrun(size_t needed) {
      mlog.debug("overrun start(%p),ptr(%p),end(%p),needed(%d)", start,ptr,end,(int)needed);
      ...
      mlog.debug("overrun start(%p),ptr(%p),end(%p),len(%d)",start,ptr,end,(int)len);
    }
