...
static rfb::LogWriter tlog("InStream");
...
namespace rdr {
  ...
  public:
    ...
    const uint8_t* GetEndPointer(){
      return end;
    }

    const uint8_t* GetPtrPointer(){
      return ptr;
    }
    ...
    inline bool hasData(size_t length) {
      ...
      checkedBytes = 0;
#endif
      tlog.debug("[InStream]hasdata length(%d), ptr(%p), end(%p),restorePoint(%p)", (int)length, ptr, end, restorePoint);
