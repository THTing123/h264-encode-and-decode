...
{
  vlog.debug("[resize] resize x(%d),y(%d),w(%d),h(%d),f_w(%d),f_h(%d)", x,y,w,h,frameBuffer->width(),frameBuffer->height());
...
int Viewport::handleSystemEvent(void *event, void *data)
{
  ...
  if ((msg->message == WM_MOUSEMOVE) ||
    ...
    vlog.debug("[Vhandle] e_x(%d),e_y(%d),e_x_r(%d)e_y_r(%d)", Fl::event_x(),Fl::event_y(),Fl::event_x_root(),Fl::event_y_root());
    vlog.debug("[handleSystemEvent] message(%d), lbutton(%d)", msg->message, WM_LBUTTONUP);
