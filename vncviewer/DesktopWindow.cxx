...
void DesktopWindow::resizeFramebuffer(int new_w, int new_h)
{
  ...
  vlog.debug("[resizeFramebuffer] new_w(%d), new_h(%d)", new_w, new_h);
  repositionWidgets();
}
...
void DesktopWindow::draw()
{
  ...
  if ((offscreen == NULL) ||
    ...
    delete offscreen;
    vlog.debug("[DesktopWindow::draw] w(%d), h(%d), o_w(%d), o_h(%d)", w(), h(),offscreen->width(),offscreen->height());
  ...
  if (offscreen) {
    vlog.debug("[DesktopWindow::draw] offscreen o_w(%d), o_h(%d)",offscreen->width(),offscreen->height());
  ...
  if (statsGraph) {
    ...
    vlog.debug("[DesktopWindow::draw] statsGraph x(%d), y(%d), w(%d), h(%d)", ox, oy, ow, oh);
  ...
  if (overlay) {
    ...
    if (fullscreen_active()) {
      ...
      vlog.debug("[draw] windowRect x(%d), y(%d), w(%d), h(%d), count(%d)", x(), y(), w(), h(), Fl::screen_count());

      bool foundEnclosedScreen = false;
      for (int i = 0; i < Fl::screen_count(); i++) {
        Fl::screen_xywh(sx, sy, sw, sh, i);

        vlog.debug("[draw] screen_xywh x(%d), y(%d), w(%d), h(%d), i(%d)", sx, sy, sw, sh, i);
...
void DesktopWindow::resize(int x, int y, int w, int h)
{
  ...
  vlog.debug("[resize] this->w(%d), this->h(%d), x(%d), y(%d), w(%d), h(%d)", this->w(), this->h(), x, y, w, h);
  if (resizing) {
...
int DesktopWindow::handle(int event)
{
  ...
  case FL_FULLSCREEN:
    ...
    vlog.debug("[handle] before repositionWidgets ");
    // Update scroll bars
    repositionWidgets();
  ...
  case FL_DRAG:
  case FL_MOVE:
    vlog.debug("[Whandle]FL_MOVE e_x(%d),e_y(%d),e_x_r(%d)e_y_r(%d),w(%d),h(%d)", Fl::event_x(),Fl::event_y(),Fl::event_x_root(),Fl::event_y_root(),w(),h());
    if (mouseGrabbed) {
